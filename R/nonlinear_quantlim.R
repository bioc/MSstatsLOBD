...
linear_quantlim = function(datain, alpha = 0.05, Npoints = 100, 
  ...
  # Need to change this to account for limited number of samples (4): # MATEUSZ: change what? Should we move this comment to github issues?
  ...
  for (j in 1:Nbootstrap) {
    ...
    while(ll < max_iter) {
      ...
      sink(); # MATEUSZ
      if(!is.null(lin.blank_B)) break # MATEUSZ: move it to while() condition?
    }
