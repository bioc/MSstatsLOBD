...
BiocManager::install("MSstatsLODQ")
```
...
df <- tbl_df(raw_data) %>%  select(Peptide.Sequence,Precursor.Charge, 
...
df <- df %>% 
  mutate(heavy.Area = as.numeric(heavy.Area)) %>% filter(!is.na(heavy.Area))
